...
SELECT  name_of_track
  FROM  track_list
 WHERE  name_of_track ILIKE 'Мое %'
    OR  name_of_track ILIKE '% Мое %'
    OR  name_of_track ILIKE '% Мое'
    OR  name_of_track ILIKE 'Mое'
    OR  name_of_track ILIKE 'my %'
    OR  name_of_track ILIKE '% my %'
    OR  name_of_track ILIKE '% my'
    OR  name_of_track ILIKE 'my';
...
LEFT JOIN  list_of_musician AS m  ON mg.id_musician = m.id_musician 
 GROUP BY  g.name_of_genre;
  ...
  -- Количество треков, вошедших в альбомы 2019–2020 годов
   ...
   SELECT  COUNT(t.id_track) FROM album_list AS a
...
LEFT JOIN  list_of_musician AS m ON ma.id_musician = m.id_musician 
 ...
 GROUP BY  c.name_of_collection;
   ...
   SELECT  DISTINCT a.name_of_album FROM album_list AS a
...
--LEFT JOIN  music_genre AS g ON mg.id_music_genre = g.id_music_genre 
 GROUP BY  a.name_of_album, mg.id_musician
   HAVING  COUNT (mg.id_music_genre) > 1;
...
LEFT JOIN  track_list  AS t ON a.id_album = t.id_album 
 GROUP BY  m.name_of_musician, t.track_duration  
   HAVING  t.track_duration = (SELECT MIN (t.track_duration)
                                FROM track_list AS t 
                                LEFT JOIN  musicians_album  AS ma ON ma.id_album = t.id_album);
